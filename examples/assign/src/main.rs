...
async fn main() {
    ...
    loop {
        ...
        let assigned_process = colonies::assign(&colonyid, false, 10, &runtimeprvkey).await;
        ...
        match assigned_process.spec.func.as_str() {
            "say" => {
                ...
                let res = colonies::add_attr(&attr, runtimeprvkey).await;
                if res.is_err() {
                    println!("failed to add attribute");
                }
                let res = colonies::close(&assigned_process.processid, runtimeprvkey).await;
                if res.is_err() {
            ...
            _ => {
                ...
                let res = colonies::fail(&assigned_process.processid, runtimeprvkey).await;
                if res.is_err() {
